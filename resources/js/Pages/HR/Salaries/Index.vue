...
<template>
    <div class="flex flex-col px-6">
        ...
        <div class="flex flex-wrap items-center justify-between mb-8 gap-4">
            <input type="search" v-model="searchQuery" placeholder="Search by name or department..."
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-x-auto">
            <table class="min-w-full text-left whitespace-nowrap">
                ...
                <tbody>
                    <tr v-for="salary in filteredSalaries" :key="salary.id" class="border-t-4 border-gray-200">
                        <td class="p-4">{{ salary.serialNo }}</td>
                        <td class="p-4">{{ salary.employee_id }}</td>
                        <td class="p-4">{{ salary.employee_name }}</td>
                        <td class="p-4 text-center">
                            ...
                            }" class="inline-block w-24 text-center py-2 rounded-full text-sm font-semibold capitalize">
                                {{ salary.role }}
                            </span>
                        </td>
                        <td class="p-4">{{ salary.department }}</td>
                        <td class="p-4">{{ formatCurrency(salary.full_salary) }}</td>
                        <td class="p-4">{{ formatCurrency(salary.net_salary) }}</td>
                        <td class="p-4 text-center">
