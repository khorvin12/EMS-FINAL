...
const props = defineProps({
    ...
    attendance: {
        ...
        default: () => ({
            ...
            total_hours_worked: 0, expected_hours: 176, undertime_hours: 0,
            overtime_hours: 0, overtime_pay: 0, present_days: 0, total_working_days: 22
        })
    },
...
const calculatedSalary = computed(() => {
    ...
    if (!props.payrollGenerated) {
        return { gross: grossSalary, dailyRate, hourlyRate, absenceDeduction: 0, lateDeduction: 0, undertimeDeduction: 0, overtimePay: 0, totalDeductions: 0, net: grossSalary };
    }
    ...
    return { gross: grossSalary, dailyRate, hourlyRate, absenceDeduction, lateDeduction, undertimeDeduction, overtimePay, totalDeductions, net: netSalary };
});
...
const generatePayroll = () => {
    ...
    router.post(`/hr/salaries/${props.employee.id}/generate-payroll`, {}, {
        ...
        onSuccess: () => {
            ...
            toast.value?.addToast('Payroll generated successfully!', 'success')
        },
        onError: () => {
            isGenerating.value = false;
            toast.value?.addToast('Failed to generate payroll. Please try again.', 'error')
        }
    });
};
...
<template>
    <div class="max-w-5xl mx-auto py-8 px-4">
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="p-8">
                <div class="bg-gray-50 rounded-xl p-6 mb-8 grid grid-cols-1 sm:grid-cols-3 gap-4 overflow-hidden">
                    <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                    <div class="bg-white rounded-lg p-6 border-l-4 border-purple-500 shadow-sm overflow-hidden">
                        <p class="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Daily Rate</p>
                        <p class="text-xl md:text-3xl font-bold text-gray-900 mb-1">₱{{
                            formatCurrency(calculatedSalary.dailyRate) }}</p>
                        <p class="text-sm text-gray-500">Gross Salary ÷ 22 days</p>
                    </div>
                    <div class="bg-white rounded-lg p-6 border-l-4 border-indigo-500 shadow-sm overflow-hidden">
                        <p class="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Hourly Rate</p>
                        <p class="text-xl md:text-3xl font-bold text-gray-900 mb-1">₱{{
                            formatCurrency(calculatedSalary.hourlyRate) }}</p>
                ...
                <div v-if="payrollGenerated"
                    class="bg-blue-50 border-l-4 border-blue-500 p-6 mb-8 rounded-lg overflow-x-auto">
                    ...
                    <div class="bg-white p-4 rounded-lg shadow-sm mb-4">
                        <div class="flex justify-between items-center">
                            <div class="overflow-hidden">
                                ...
                                <p class="text-2xl font-bold text-orange-600">{{ attendance.total_hours_worked }} / {{
                                    attendance.expected_hours }} hours</p>
                                <p class="text-xs text-gray-500 mt-1">({{ attendance.present_days }} of {{
                                    attendance.total_working_days }} working days completed)</p>
                            </div>
                            <div v-if="attendance.undertime_hours > 0" class="text-right">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Deduction: ₱{{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 overflow-hidden">
                        ...
                        <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-green-400">
                            ...
                            <p class="text-xs text-gray-500 mt-1">+₱{{ formatCurrency(calculatedSalary.overtimePay) }}
                                (1.25x)</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div
                        class="bg-white rounded-xl shadow-sm p-6 border-l-4 border-green-500 flex flex-col justify-between hover:shadow-md transition">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Gross Salary</p>
                        <p class="text-3xl md:text-4xl font-extrabold text-gray-900 break-words">₱{{
                            formatCurrency(calculatedSalary.gross) }}</p>
                        <p class="text-sm text-gray-500 mt-2">Monthly base</p>
                    </div>
                    <div
                        class="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500 flex flex-col justify-between hover:shadow-md transition">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Total Deductions</p>
                        <p class="text-3xl md:text-4xl font-extrabold text-gray-900 break-words">₱{{
                            formatCurrency(calculatedSalary.totalDeductions) }}</p>
                        <div class="text-xs space-y-1 mt-2">
                            <p v-if="!payrollGenerated" class="text-gray-400 italic">Pending generation...</p>
                            <template v-else>
                                <p v-if="attendance.absences > 0" class="text-red-600">Absence: -₱{{
                                    formatCurrency(calculatedSalary.absenceDeduction) }}</p>
                                <p v-if="attendance.late_count > 0" class="text-orange-600">Late: -₱{{
                                    formatCurrency(calculatedSalary.lateDeduction) }}</p>
                                <p v-if="attendance.undertime_hours > 0" class="text-red-600">Undertime: -₱{{
                                    formatCurrency(calculatedSalary.undertimeDeduction) }}</p>
                                <p v-if="attendance.overtime_hours > 0" class="text-green-600">Overtime: +₱{{
                                    formatCurrency(calculatedSalary.overtimePay) }}</p>
                                <p v-if="calculatedSalary.totalDeductions === 0" class="text-green-600 font-bold">
                                    Perfect Attendance ✓</p>
                            </template>
                        </div>
                    </div>
                    <div
                        class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl shadow-lg p-6 text-white flex flex-col justify-between hover:shadow-xl transition">
                        <p class="text-xs font-semibold uppercase tracking-wide mb-2 opacity-90">Net Salary</p>
                        <p class="text-3xl md:text-4xl font-extrabold break-words">₱{{
                            formatCurrency(calculatedSalary.net) }}</p>
                        <p class="text-sm opacity-90 mt-2">Estimated take-home pay</p>
                    </div>
                </div>

                <div v-if="payrollGenerated" class="mt-8 flex justify-center whitespace-nowrap">
                    <button @click="generatePayroll" :disabled="isGenerating"
